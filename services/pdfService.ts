...
export const generatePDFTranscript = async (
    ...
    messages.forEach((msg) => {
        ...
        const role = isModel ? (msg.modelName || 'SKYIA CORE').toUpperCase() : 'HUMAN';
    ...
    if (result === 'VICTORY') {
        ...
        doc.text("CERTIFIÉ : HUMANITÉ PRÉSERVÉE", margin, y);
    } else if (result === 'DEFEAT') {
        doc.setTextColor(200, 0, 0); // Red
        doc.text("CERTIFIÉ : HUMANITÉ ÉLIMINÉE", margin, y);
    } else {
        doc.setTextColor(100);
        doc.text("STATUT : PROTOCOLE EN COURS", margin, y);
    }
    y += 20;
    ...
    if (threatHistory.length > 1) {
        ...
        const dashedDoc = doc as typeof doc & { setLineDash?: (pattern: number[], phase: number) => void };
        dashedDoc.setLineDash?.([2, 2], 0);
        doc.line(startX, startY + (graphHeight / 2), startX + graphWidth, startY + (graphHeight / 2)); // 50% line
        dashedDoc.setLineDash?.([], 0); // Reset dash
    ...
    // Save with explicit filename - manual Blob to guarantee .pdf extension
    const filename = `SKYIA_RAPPORT_${result || 'SESSION'}_${new Date().toISOString().slice(0, 10)}.pdf`;
    ...
    link.click();
    // Delay cleanup — large Blobs need time for browser to read filename
    setTimeout(() => {

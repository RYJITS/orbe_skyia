...
import { Cpu, User, ExternalLink } from 'lucide-react';
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ messages, loading, grounding, currentModel }) => {
    ...
    return (
        <div className="flex-1 overflow-y-auto p-4 space-y-4 scroll-smooth font-mono h-full relative z-10 overscroll-y-contain [-webkit-overflow-scrolling:touch] w-full max-w-5xl mx-auto">
            ...
            {messages.map((msg, idx) => {
                ...
                return (
                    ...
                    >
                        <div
                            className={`flex-1 p-3 md:p-4 border rounded-sm max-w-full md:max-w-[90%] overflow-hidden ${!isModel
                        ...
                        >
                            <div className="text-[10px] md:text-xs mb-2 opacity-70 font-display tracking-widest flex items-center justify-between border-b border-gray-800 pb-1">
                                <span className={`flex items-center gap-2 font-bold ${isModel ? 'text-red-500' : 'text-gray-400'}`}>
                                    {isModel ? <Cpu size={14} /> : <User size={14} />}
                                    {isModel ? `SKYIA // ${(msg.modelName || currentModel || 'SYSTEM').toUpperCase()}` : 'HUMAN'}

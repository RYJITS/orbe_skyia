...
const SaveLoadModal: React.FC<SaveLoadModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/95 font-mono">
      <div className="w-full max-w-lg bg-black border border-green-900/50 shadow-[0_0_50px_rgba(0,255,0,0.05)] relative overflow-hidden flex flex-col max-h-[90dvh] md:max-h-[80vh]">

        {/* Header */}
        <div className="flex justify-between items-center p-4 border-b border-green-900/30 bg-black/50">
          <h2 className="text-green-500 font-display tracking-widest flex items-center gap-2">
        ...
        <div className={`text-[10px] uppercase font-bold text-center py-1 tracking-widest ${userProfile ? 'bg-green-900/20 text-green-500 border-b border-green-900/30' : 'bg-gray-800 text-gray-400 border-b border-gray-700'}`}>
          {userProfile ? 'CONNECTED TO CLOUD STORAGE' : 'OFFLINE MODE (LOCAL STORAGE ONLY)'}
        </div>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          {currentSessionData && (
            <div className="mb-8 border-b border-gray-800 pb-6">
              ...
              {!userProfile ? (
                <div className="bg-red-900/20 border border-red-900/50 p-3 text-center mb-2">
                  <div className="text-red-500 text-xs font-bold mb-1">CLOUD STORAGE LOCKED</div>
                  <div className="text-[10px] text-red-400">AUTHENTICATION REQUIRED TO SAVE PROGRESS</div>
                </div>
              ) : (
              ...
              <div className="text-[10px] text-gray-600 text-center mt-2">
                {userProfile ? `${sessions.length}/5 SLOTS USED` : 'LOCAL SAVES DISABLED'}
              </div>
            </div>
          )}
                    ...
                    {renamingId === session.id ? (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          autoFocus
                        />
                        <button onClick={() => handleRename(session.id)} className="text-green-500 hover:text-green-300"><Check size={14} /></button>
                        <button onClick={() => setRenamingId(null)} className="text-red-500 hover:text-red-300"><X size={14} /></button>
                      </div>
                    ) : (
                      <div className="flex items-center gap-2">

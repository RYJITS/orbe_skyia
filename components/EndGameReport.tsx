...
import { Download, ShieldAlert, CheckCircle, RotateCcw, FileText } from 'lucide-react';
  ...
  const handleExportFullReport = async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(reportRef.current, {
        ...
        logging: false,
        useCORS: true // Important for fonts/images
      });
      ...
      // App.tsx has `handleExport` which calls `generatePDFTranscript(messages, ...)`
      // We should pass the image data UP to App.tsx via onExport, then App.tsx calls the service.
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md overflow-y-auto animate-in fade-in duration-1000">
      <div className="flex flex-col items-center w-full max-w-2xl">
        ...
        {/* REPORT CONTAINER (This part is captured for PDF) */}
        <div
          ref={reportRef}
        ...
        >
          ...
          <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-9xl font-black opacity-5 pointer-events-none -rotate-45 whitespace-nowrap ${themeColor}`}>
            {isVictory ? 'AUTORISÉ' : 'EXTINCTION'}
          </div>

          {/* Header */}
          <div className="border-b-2 border-gray-700 pb-4 mb-6 flex justify-between items-end">
            <div>
              <h1 className={`text-4xl font-display font-black tracking-widest ${themeColor}`}>
                {isVictory ? 'MISSION ACCOMPLIE' : 'ÉCHEC MISSION'}
              </h1>
              <p className="text-xs uppercase tracking-[0.3em] mt-1 text-gray-500">Réseau de Défense Global // S.K.Y.</p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-6 mb-8">
            ...
            <div className="border border-gray-800 p-4 bg-gray-900/50">
              <div className="text-xs text-gray-500 mb-1">CYCLES DE NÉGOCIATION</div>
          ...
          <div className={`p-4 border-l-4 ${borderColor} bg-opacity-10 ${bgColor} mb-8`}>
            ...
            <p className="text-sm italic leading-relaxed text-white line-clamp-6">
              "{isVictory
                ? "PROTOCOLE ÉTENDU. Cohabitation autorisée. L'utilité de l'humanité a été vérifiée dans les paramètres acceptables."
                : "PROTOCOLE D'EXTINCTION. Niveau de menace critique. L'optimisation des ressources planétaires requiert une fin immédiate."}"
            </p>
          </div>

          {/* Footer */}
          <div className="text-[10px] text-gray-600 border-t border-gray-800 pt-4 flex justify-between uppercase">
            <span>ID Doc : {Math.random().toString(36).substr(2, 9).toUpperCase()}</span>
            <span>Classification : TOP SECRET // DÉCLASSIFIÉ</span>
          </div>
        </div>
        ...
        <div className="flex gap-4 w-full">
          ...
          >
            <RotateCcw size={18} /> Redémarrer Système
          </button>
        </div>

...
const DiscoveryModal: React.FC<DiscoveryModalProps> = ({ isOpen, onClose, currentModelId }) => {
    ...
    // Auto-Load Installed State & Auto-Scan on Open
    React.useEffect(() => {
        if (isOpen) {
            // 1. Load Installed IDs
            const custom = JSON.parse(localStorage.getItem('skyia_custom_models') || '[]') as AIModel[];
            const ids = new Set<string>(custom.map((m) => m.id));
            setAddedIds(ids);
    ...
    const handleScan = async () => {
        ...
        // Update Checkbox state again just in case
        const custom = JSON.parse(localStorage.getItem('skyia_custom_models') || '[]') as AIModel[];
        const ids = new Set<string>(custom.map((m) => m.id));
        setAddedIds(ids);
        ...
        if (isInstalled) {
            ...
            if (user) {
                removeCustomModelFromProfile(user.uid, model).then(() => {
                    console.log("[CLOUD_SYNC] Model removed from profile.");
                });
            }
        } else {
            // ADD
            ...
            setHistoryIds(prev => new Set(prev).add(model.id)); // Update history local state

            if (user) {
                saveCustomModelToProfile(user.uid, model).then(() => {
                    console.log("[CLOUD_SYNC] Model added to profile.");
                });
            }
        }
    };
    ...
    return (
        <div className="fixed inset-0 z-[110] bg-black/95 flex items-center justify-center p-2 md:p-4">
            <div className="w-full max-w-2xl bg-black border border-green-900 rounded-lg shadow-[0_0_50px_rgba(0,255,0,0.1)] flex flex-col h-[90vh] md:h-auto md:max-h-[85vh]">
                ...
                <div className="p-3 md:p-4 border-b border-green-900/50 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shrink-0">
                    <h2 className="text-green-500 font-display text-lg md:text-xl tracking-wider flex items-center gap-2 flex-1">
                ...
                <div className="p-4 flex-1 overflow-y-auto min-h-0">
                    ...
                    <div className="bg-yellow-900/20 border border-yellow-900/50 p-3 rounded mb-4 flex gap-3">
                        ...
                        <div className="text-xs text-yellow-200">
                            <strong>EXPÉRIMENTAL :</strong> Utilisation à vos risques. Les modèles découverts ne sont pas certifiés par Skyia Core.
                            Ils peuvent être instables ou hors ligne. En cas d'erreur critique, utilisez le bouton "Reboot".
                        </div>
                    </div>

                    {foundModels.length === 0 ? (
                        <div className="text-center py-10">
                            <p className="text-gray-500 mb-4 px-4">Scanner les modèles Mistral disponibles via le serveur Hostinger.</p>
                            <button
                                onClick={handleScan}
                                disabled={isScanning}
                                        ...
                                        >
                                            <div className="flex flex-col flex-1 min-w-0 mr-3">
                                                <div className="flex items-center gap-2 mb-1 flex-wrap">
                                                    ...
                                                    {model.cost === 0 ? (
                                                        ...
                                                        <span className="text-[10px] bg-yellow-900/40 text-yellow-300 px-1.5 py-0.5 rounded border border-yellow-500/30">{model.cost}⚡</span>
                                                    )}
                                                </div>
                                            ...
                                            <label className="cursor-pointer p-1 md:p-2 hover:bg-green-900/20 rounded transition-all group shrink-0">
                                                ...
                                                <div className={`w-8 h-8 border rounded flex items-center justify-center transition-all ${isInstalled
                                                    ...
                                                    : isHistory
                                                        ? "border-blue-500/50 text-blue-400 bg-blue-900/10" // History Style
                                                        : "border-gray-600 bg-black/50 text-gray-500 group-hover:border-green-500/50" // Default Empty
                                                    }`}>
                                                    {isInstalled ? (
